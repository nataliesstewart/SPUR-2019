...
Let $\{T_i\}$ be the transpositions generating the Hecke algebra $\SH_{2n+r}(q)$. Let $W_{2n+r}^r$ be the generalized crossingless matchings representation with $2n+r$ nodes, $r$ of which are anchors. Fix the standard basis; we will refer to no other basis in this document. Here we characterize the intersection of the kernels of each $(1+T_i)$, a subrepresentation of $W_{2n+r}^r$. I claim this intersection is trivial for $e>n+r+1$ and one dimensional for $e=n+r+1$.
...
\begin{definition}
  ...
  Fix some basis element $\psi\in W_{2n+r}^r$. For $1 \leq a,b \leq 2n + r$ define $\psi(a):=b$ iff $a$ and $b$ are matched in $\psi$, $\psi(a):=a$ if $a$ is an anchor in $\psi$. Given that $\psi$ has $r'$ anchors in the range $a,..,b$, define a \textbf{sub-matching} $\psi(a,b)$ of $\psi$ to be the basis element $\sigma\in W_{b-a+1}^{r'}$ specified by $\sigma(i)=\psi(i+a-1)-a+1$. This sub-matching is defined for $a<b$ when $\psi(i)\in \{a,a+1,...,b\}$ for all $i\in \{a,a+1,...,b\}$. See Figure 1.
...
\begin{figure} 
	...
	$\cbr{
		...
		\Matching{4}{1/4, 2/3}
		\end{gathered}}$
	\caption{$\psi\in W_6^0$ is pictured on the left, $\sigma\in W_4^0$ is pictured on the right. $\psi(3,6)=\sigma$. $\psi(2,5)$ is not a sub-matching.}
	\label{S6 Basis}
\end{figure}
...
Given a rainbow element $R$, define the basis elements $R_{R,i},R_{L,i}$ to be those where you move the middle hump across $i$ lines to the right or left, respectively. Examples are pictured in figure 2. Formally, $R_{R,i}:=q^{-i/2}(1+T_{r+n+i})...(1+T_{r+n+1})R$, $R_{L,i}:=q^{-i/2}(1+T_{r+n-i})...(1+T_{r+n-1})R$.
...
$$\frac{(-1)^{n+1}(q^{n+1}+2q^n+...+2q+1)}{q^{(n+1)/2}}-\frac{(-1)^{n+1}(q^n+...+q)}{q^{(n+1)/2}}=\frac{(-1)^{n+1}(q^{n+1}+...+1)}{q^{(n+1)/2}}=Q_{n+1}$$.
...
\begin{proof}
	...
	Let $n:=a+b-1$ and $r$ be the number of anchors in $y(a,b)$. Consider the coordinate of $\phi_i$ in $(1+T_{a-1+r+n/2-i})w$. This is the transposition that acts on the "moved middle hump" in $\phi_i(a,b)=R_{L,i}$, as shown in figure 3. I claim the following:
	...
	\begin{figure}
		...
		\]
    \caption{The action of $(1+T_{a-1+r+n/2-i})$ on $\phi_i,\phi_{i=1},\phi_{i+1}$ (ordered from top to bottom), shown as the case where $y$ is the rainbow vector in $W_{8}^2$ and $i=2$.}
		\label{Action}
	\end{figure}
\end{proof}	
...
%	For our base case we will prove $b=2$. This means either $a_1+1$ is an anchor, or $\beta(a_1+1)=2n+r$. 
%	\\
%	
%	First base case:
%	
%	If $a_1+1$ is an anchor, to still be in case 3 we must have $i=1$ and $a_1=2$. In this case, lemma 3.7 gives that there is only one element in $E_\beta$, and that element is $\alpha$ defined by $\alpha(4,2n+r)=\beta(4,2n+r)$, $\alpha(1)=1$, and $\alpha(2)=3$. Then $x_\beta=Q_{n+r-1}x_\beta(3,2n+r)=Q_{n+r-1}\beta(4,2n+r)$ and $x_\alpha=x_{\alpha(2,2n+r)}=Q_{n+r-2}x_{\beta(4,2n+r)}$. So, using lemma 2.4, we have $(q+1)x_\beta(q)+q^{1/2}x_\alpha(q)=-q^{1/2}x_{\beta(4,2n+4)}(q)Q_{n+r}$ as desired.
%	\\
%	
%	Second base case: 
%	
%	If $\beta(a_1+1)=2n+r$, to be in case 3 we need either $a_1=2$ or $a_1=2n+r-2$. In both cases lemma 3.7 gives only one element $\alpha$ in $E_\beta$. In the first case $x_\beta=Q_{n+r-1}x_{\beta(3,2n+r)}=Q_{n+r-1}x_{\beta(4,2n+r-1)}$, and $x_\alpha=x_{\alpha(2,2n+r-1)}=Q_{n+r-2}x_{\beta(4,2n+r-1)}$ so again using lemma 2.4 $(q+1)x_\beta(q)+q^{1/2}x_\alpha(q)=-q^{1/2}x_{\beta(4,2n+r)}(q)Q_{n+r}$ as desired. In the second case $x_\beta=\frac{Q_{n+r-1}...Q_{1}}{Q_1...Q_{n+r-2}}x_{\beta(2,2n+r-3)}=Q_{n+r-1}x_{\beta(2,2n+r-3)}$, and, using corollary 3.3, $x_\alpha=\frac{Q_{n+r-2}...Q_{1}}{Q_1...Q_{n+r-3}}x_{\alpha(2,2n+r-3)}=Q_{n+r-2}x_{\beta(2,2n+r-3)}$, so again using lemma 2.4, we have $(q+1)x_\beta(q)+q^{1/2}x_\alpha(q)=-q^{1/2}x_{\beta(1,2n+r-2)}(q)Q_{n+r}$ as desired, and all base cases are proved.
%	\\
