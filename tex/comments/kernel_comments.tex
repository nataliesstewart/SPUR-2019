...
{\color{magenta} This is more of a thing to discuss when integrating into the draft, but this needs to be restructured in order to fit in.}
...
{\color{magenta} May just be a me thing, but I find the amount of vspace and the centering on the proposition statements to be visually confusing.}

\vspace{5mm}
\begin{proposition}
	
  Let $W_{2n+r}^r$ be a crossingless matchings representation, and suppose $Q_1,...Q_{n+r-1},\not=0$. Let $w\in\cap\text{ker}(1+T_i)$. WLOG {\color{magenta} assume} the rainbow element $R$ has coordinate 1 in $w$ (by proposition 2.2).
	\\
...
\end{proposition}
{\color{magenta} This may be a stylistic thing, but the phrase ``I claim'' in a written proof feels weird to me.}
\vspace{5mm}
Before proving this proposition, it will be useful to clarify exactly what it states.

{\color{magenta} I'm not going to mention it again, but this vspacing is visually confusing to me.
I've been meaning to bring this up because I'm not exactly sure how to use this;
I generally use it sparingly (not often and $\leq$ line height) to separate large chunks in proofs.
It's fair if you like it this way, but I'd be a bit confused reading this.}
...
{\color{magenta} Is this a bit redundant? This just seems like the whole statement.}
...
\end{enumerate}
{\color{magenta} I feel this would be better in a paragraph than an enumerated list;
it isn't complicated enough that a list really does anything but give an informal tone IMO.}
\vspace{10mm}
\begin{proof}
	...
	By proposition 2.2 the rainbow element has coordinate 1 in $w\in W_{2n+r}^r$. 
  {\color{magenta} Wasn't this already stated in the proposition?}
  By proposition 2.3 the element $R_1:=R_{L,n+r-1}$ has coordinate $Q_{n+r-1}$ in $w$. Then $R_1(3,2n+r)$ is the rainbow element in  $W_{2(n-1)+r}^r$, so the element $R_2$ defined by $R_2(1,2)=R_1(1,2)$, $R_2(3,2n+r)=R_{L,n+r-2}\in W_{2(n-1)+r}^r$ has coordinate $Q_{n-1}Q_{n-2}$. 
  {\color{magenta} the element $R_2$ defined by submatchings $R_2(1,2) := R_1(1,2)$ and $R_2(3,2n + r) := $\dots}
  Generally, define $R_i$ by $R_i(1,2(i-1))=R_{i-1}(1,2(i-1))$, $R_i(2i-1,2n+r)=R_{L,2n+r-i}$. Then the coefficient of $R_i$ is $Q_{n+r-1}...Q_{n+r-i}$. These elements are shown in figure 5.
  ...
  {\color{magenta} If it is significant enough to be stated as ``I claim'', it may be better put in the claim* environment, or as a lemma.}
  ...
  {\color{magenta} Suppose inductively that\dots}
  ...
  {\color{magenta} Two comments on the inductive arument:
    \begin{enumerate}
      \item IMO inductive arguments flow better when they begin with the base case.
      \item I don't think that the verbose treatment of the induction argument is necessary or helpful;
        one actually needs to use the word induction once.
        I think a good structure is ``We will prove indutively that \dots For the base case $2n + r = 2$ \dots suppose \textit{indutive hypothesis}\dots qed'', which doesn't require one to remind the reader that they're reading an inductive argument.
    \end{enumerate}
}
\end{proof}

{\color{magenta} I do like this proof above, and I like the clear sectioning.
I think it might even be nice to see a \textit{Step n} at the beginning of each step as well as a small vspace for separation.}

\vspace{5mm}
The following few corollaries will help to simplify some later proofs.
{\color{magenta} ``some later arguments'' (``proofs'' feels like it breaks the tone).}
...
\begin{proof}
  ...
  {\color{magenta} the align* environment would help this a lot.}
  ...
  {\color{magenta} I don't think this needs to be said.}
\end{proof}

\vspace{5mm}
\begin{corollary}
  {\color{magenta} if $\beta \in W_{2n + r}^r$ is a basis element\dots} $\beta\in W_{2n+r}^r$, then $x_\beta(q)\not=0$ if $e>n+r$.
\end{corollary}
...
{\color{magenta} I like this section a lot, and especially how the argument unfolds with the corollaries.
One comment I have is that the separation between this and the next section would be handled much better with subsections than a horizontal line.}

\vspace{5mm}
\noindent\rule{16.5cm}{0.4pt}
{\color{magenta} The rest of this seems technical. I'll pick it back up tomorrow morning.}
...
The next lemma characterizes cases where the transposition is not under any arcs and all anchors are to the right.
 ...
 \begin{figure}[b]
    ...
    {\color{magenta} This will be handled when we fully assemble the draft, but the figures should be made to fit the margins via adjustbox if possible}
  }
	%\label{Action}
\end{figure}
