	...
	def __init__(self, n, match):
		...
		for i in match.keys():
			...
			for j in range(min(i,match[i])+1,max(i,match[i])):
				assert min(i,match[i])<match[j] and max(i,match[i])>match[j], 'matchings cross'
		self.match=match
...
class module_element:
	...
	def mult_generator(self, i):
		...
		for basis in self.vecpairs.keys():
			if basis.pair(i)==i+1:
				new=new+module_element(self.n, {basis:self.vecpairs[basis]*sqrt(self.q)},self.q)
			else:
				...
				new=new+module_element(self.n, {newbasis:self.vecpairs[basis]*(1 + self.q)},self.q)
		return new
